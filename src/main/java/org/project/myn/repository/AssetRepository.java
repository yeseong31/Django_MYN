...
public interface AssetRepository extends JpaRepository<Asset, Long> {
    ...
    @Query("select a, ac from Asset a, Account ac where a.id = :id and a.owner.id = ac.id")
    List<Object[]> getAssetWithAccount(@Param("id") Long id);

    // '/explore'의 'New Assets'에 해당하는 정보: 가장 최근에 등록된 Asset 정보와 username 정보 조회
    @Query("select a, ac.username from Asset a left outer join Account ac on ac.id = a.owner.id")
